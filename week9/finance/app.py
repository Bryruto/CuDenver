...
def buy():
    if request.method == "GET":
        ...
        if shares < 0:
            return apology("need some shares")
        ...
        db.execute("INSERT INTO transactions (user_id,symbol,shares,price,date) VALUES (?,?,?,?,?)",user_id,symbol,shares,transaction_value, date)
...
def sell():
    if request.method == "GET":
        user_id = session["user_id"]
        symbols_user = db.execute("SELECT symbol FROM transactions WHERE user_id = ? GROUP BY symbol HAVING SUM(shares)",user_id)
        ...
        if shares > user_shares_real:
            return apology("you do not have those shares")
        ...
        db.execute("INSERT INTO transactions (user_id,symbol,shares,price,date) VALUES (?,?,?,?,?)",user_id,symbol,(-1*shares),transaction_value, date)
